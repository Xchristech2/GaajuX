...
const store = {
    ...
    readFromFile(filePath = STORE_FILE) {
        try {
            ...
            // Recover from corrupted persisted store instead of crashing future boots
            this.contacts = {}
            this.chats = {}
            this.messages = {}
            try {
                if (fs.existsSync(filePath)) {
                    ...
                    console.warn(`Corrupted store moved to ${brokenPath}`)
                }
            } catch (moveErr) {
                console.warn('Failed to quarantine corrupted store file:', moveErr.message)
            }
        }
    },
